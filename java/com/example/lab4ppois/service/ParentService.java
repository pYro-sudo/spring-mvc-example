...
public class ParentService {
    ...
    @Caching(
            evict = {
                    @CacheEvict(value = "parent-id", key = "#parentId"),
                    @CacheEvict(value = "parent")
            }
    )
    public Mono<List<Child>> getKidsToSchool(String parentId, String classroomId) {
        return parentRepository.findById(parentId)
                .flatMap(parent -> classroomService.getClassroomById(classroomId)
                        .map(parent::getKidsToSchool)
